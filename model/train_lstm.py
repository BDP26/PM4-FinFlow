"""
Distributed training of a univariate LSTM model on 4 GPUs using Ray Train.
Predicts next-month fishing hours from a 12-month lookback window.
"""
import torch
...
# Generate (X, y) sliding-window pairs on-the-fly using NumPy's C-level operation
# This avoids materialising a huge intermediate dataset in RAM
...
# Train set was precomputed; validation set generates sequences on the fly
...
def train_loop_per_worker(config):
    ...
    model = ray.train.torch.prepare_model(model)  # wraps for distributed training
    ...
    for epoch in epoch_iterator:
        ...
        for batch in train_shard.iter_torch_batches(batch_size=batch_size, local_shuffle_buffer_size=10000):
            X_batch = batch["X"].float().unsqueeze(-1)  # add feature dim: (B, 12) -> (B, 12, 1)
        ...
        if world_rank == 0:
            ...
            pd.DataFrame(history).to_csv(f'{config["output_path"]}/training_history.csv', index=False)
